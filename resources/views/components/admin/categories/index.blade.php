...
new class extends Component {
    ...
    public function save()
    {
        $this->validate([
            ...
            'slug' => ['required', 'max:255', Rule::unique('categories', 'slug')->ignore($this->editing?->id)],
                        ...
                        <tr wire:key="category-{{ $category->id }}">
                            ...
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end gap-1">
                                    <button type="button" wire:click="edit({{ $category->id }})" class="btn btn-sm btn-light text-muted" title="Editar">
                                    ...
                                    <button
                                        ...
                                        x-on:click="Swal.fire({
                                            title: '¿Estás seguro?',
                                            text: 'Vas a eliminar la categoría ' + @js($category->name) + '. Se quitará de los {{ $category->posts_count }} post(s) que la usan, pero esos posts no se eliminan.',
                                            icon: 'warning',
                                        ...
                                        }).then((result) => { if (result.isConfirmed) { $wire.delete({{ $category->id }}) } })"
        ...
        <div @click.away="$wire.showModal = false"
            ...
            <form wire:submit="save">
                <div class="d-flex justify-content-between align-items-center p-3 border-bottom">
                    <h5 class="mb-0 fw-bold">{{ $editing ? 'Editar categoría' : 'Agregar categoría' }}</h5>
